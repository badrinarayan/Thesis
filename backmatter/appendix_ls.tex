...
spectral signals from previous work, which is used in the analysis in
Chapter~\ref{chap:linespect}. For some of these theorems, I also derive some
useful corollaries for some of these theorems which we used. In addition to
Theorem~\ref{dual-stab}, we will need another result in~\cite{cg_noisy} where
the authors show the existence of a trigonometric polynomial $Q_1$ that is
linear in each $N_j$ which is also an essential ingredient in the proofs.
...
\end{lemma}
\begin{proof}
I will give a detailed proof of \eqref{QL1}, and list the necessary modifications for proving \eqref{Q1L1} and \eqref{QjL1}. The dual polynomial $Q(f)$ constructed in \cite{cg_exact12} is of the
form
\begin{eqnarray}
...
\end{eqnarray}
We have already established that $\| G ( f) \|_2^2 \leq C / {n}.$ Let us now show that $\| G' ( f) \|_2^2 \leq C' {n}$. Differentiating the
...
\end{align*}
The penultimate equality follows from Parseval's theorem and the last inequality
is an application of H\"{o}lder's inequality. Then, the result follows by using
Lemma~\ref{l1} and \eqref{errbd}.
\end{proof}
% 
