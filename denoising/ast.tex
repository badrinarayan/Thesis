...
The foundation of our technique consists of extending recent work on
\emph{atomic norms} in linear inverse problems in \cite{crpw}. In this work,
...
known to be a low rank matrix, $\A$ could be the set of all unit norm rank-$1$
matrices.
...
Lasso~\cite{tibshirani96,chen01} to infinite dictionaries.
...
Suppose our target signal $x^\star \in \R^n$ is composed only of atoms from a
small subset $S \subset \A$ of the original \emph{dictionary} of atoms. We call
$S$, a \emph{support} of $x^\star$. If the number of elements $k = |S|$ in the
support is small relative to the dimension $n$ of the signal, we say $x^\star$
is a $k$ \emph{simple} combination of atoms. We emphasize that the \emph{atomic 
set} $\A$ could be infinite.

This notion of simplicity generalizes sparsity for many structures in high
dimension. For instance, the rank of a matrix is a notion of simplicity and
indeed a low rank matrix in a high dimensional space can be written as a
combination of a few elements from the infinite atomic set of all unit norm,
rank-$1$ matrices. Similarly, we can attach a notion of simplicity to a vector
$x \in \R^n$ as the smallest number of elements from a dictionary $\mathcal{D}$
of $p$ elements needed to express $x$ as a linear combination. I don't believe this.


\subsection{Decomposition and Atomic Norm}


Can we recover this sparse decomposition and find the composing atoms from
$x^\star$? As stated, the problem is ill-posed, for there could many
decompositions. If we assume that every collection of $n$ atoms from $\A$ is
linearly independent, there is a unique sparsest decomposition of $x^\star$
provided there is a decomposition that is at most $\ceil{n/2}$ sparse. Such
a dictionary of atoms is called a full spark frame and we shall use this
...
\section{Atomic Soft Thresholding}
...
\end{align*}
where the first infimum follows by completing the squares and the second infimum follows from \eq{conjugate-norm}. Thus the dual problem  of maximizing $g(z)$ can be written as in \eq{dual-ast}.
...
\todo{Copy the proof of this from TeXmacs and merge it with ideas in dual certificate section}
...
\]
This gives the desired lower bound. \todo{phrasing} We have therefore shown that in this case $\phi_\gamma(x^\star,\A)
...
\todo{Semi-infinite Programming and LP Perturbations}
...
\todo{Say stuff about Gaussian width here.}
