...
\todo{Modify phrasing for DAST} We show that solving the Lasso problem on an
oversampled grid of frequencies approximates the solution of the atomic norm
...
is a $k$ \emph{sparse} combination of atoms. We emphasize that the \emph{atomic 
set} $\A$ could be infinite.

What we just described is a general notion of sparsity which subsumes many 
simple structures in high dimension. For instance, a low rank matrix in a high 
dimensional space can be considered sparse with respect to the infinite 
dictionary of all unit norm, rank-$1$ matrices.
