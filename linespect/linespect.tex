...
The first result is a global error rate that holds for line spectral signals by
...
conditions~\cite{cg_exact12}. This frequency localization technique is later
...
Section~\ref{sec:denoise-trig-moments} describes how we can approach line
...
%, and describe the methods % on
% which our analysis is built upon.

% We then in Section~\ref{sec:atomic-norms} %
...
% % spectral estimation in Section~\ref{sec:minimax}. We then provide the proofs
% of % our main results in Section~\ref{sec:proofs}. Finally, in %
...
\end{equation}
A recent result by Candes and Fernandez-Granda~\cite{cg_exact12} establishes
...
\left( \int_F{|\nu|(df)} + I_0 + I_1 + I_2\right)
\end{equation}
As a consequence of the choice of $\tau$ given by \eq{tau}, we can show that $\vnorm{E(f)}_\infty \leq (1+2\eta^{-1})\tau$ with high probability. In fact, we have
\begin{align*}
...
\begin{proof} \label{apx:pf:I0I1} Consider the polar form \[ \int_{N_j} \nu (
...
Putting together Lemmas \ref{part1}, \ref{part2} and \ref{part3}, we can finally prove our main theorem:
\begin{align*}
...
fifth by the choice of $\tau$ according to Eq. \eq{tau}. This completes the
...
We will upper bound the first term using Lemma~\ref{l4} in Appendix \ref{apx:collection} which yields
\[
...
Directly working in the continuous parameter space circumvents the problems in
...
grid. Furthemore, Cand\`es and Fernandez-Granda~\cite{cg_exact12} showed that
...
This work is closely related to recent results established by Cand\`es and
...
This chapter demonstrated stability of atomic norm regularization by
...
%  This work provides several interesting
